...
parser.add_argument('--init_backbone', type=str, default=None, choices=['imp', 'rsp', 'none', 
                                                                        'mae', 'beit', 'samrs-mae',
                                                                        'samrs-mae-norm-pix'], help='init model')
# input img size
...
if args.dataset == 'potsdam':
    ...
    tes_dataset = ISPRSDataset(img_size = None, split='test', data_root=root, transform=None)
elif args.dataset == 'vaihingen':
    classes = 5
    crop_h = IMAGE_SIZE
    crop_w = IMAGE_SIZE
    ...
    tes_dataset = ISAIDDataset(img_size = None, split='test', data_root=root, transform=None)
else:
    raise NotImplementedError

args.image_size = IMAGE_SIZE

test_loader = D.DataLoader(tes_dataset, batch_size=1, shuffle=False, num_workers=args.workers, pin_memory=True)
