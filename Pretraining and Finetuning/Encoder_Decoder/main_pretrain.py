...
parser.add_argument('--backbone', type=str, default=None, choices=['resnet50','swint','vitaev2_s','vit_b_rvsa',
                                                                   'internimage_t','vitadapter_b','vit_b'], help='backbone name')
...
parser.add_argument('--init_backbone', type=str, default=None, choices=['imp', 'rsp', 'none', 
                                                                        'mae', 'beit','samrs-mae-expand'], help='init model')
...
elif 'vitadapter_' in args.backbone:

    from mmengine.optim import build_optim_wrapper
    from mmcv_custom.layer_decay_optimizer_constructor_vitadapter import *
    # AdamW optimizer, no weight decay for position embedding & layer norm in backbone
    optim_wrapper = dict(
        optimizer=dict(
        type='AdamW', lr=6e-5, betas=(0.9, 0.999), weight_decay=0.01),
        constructor='LayerDecayOptimizerConstructor_ViTadapter', 
        paramwise_cfg=dict(
            num_layers=12, 
...
def validation(args, logger, epoch, model, valid_loader_sota, valid_loader_sior, valid_loader_fast):
    ...
    for (i, ((x1, y1),(x2,y2),(x3,y3))) in enumerate(zip(valid_loader_sota, valid_loader_sior, valid_loader_fast)):
        ...
        x1, y1, x2, y2, x3, y3 = x1.to(DEVICE), y1.long().to(DEVICE), x2.to(DEVICE), y2.long().to(DEVICE), x3.to(DEVICE), y3.long().to(DEVICE)
...
while True:
    ...
    for ((x1, y1), (x2, y2), (x3, y3)) in zip(train_loader_sota, train_loader_sior, train_loader_fast):
        ...
        x1, y1, x2, y2, x3, y3 = x1.cuda(), y1.long().cuda(), x2.cuda(), y2.long().cuda(), x3.cuda(), y3.long().cuda()
        ...
        #if (iter > 10000 and iter % 5000 ==0) or (iter < 10000 and iter % 10 ==0):
