...
class ViTAE_Window_NoShift_basic(nn.Module):
    def __init__(self, img_size=224, in_chans=3, stages=4, embed_dims=64, token_dims=64, downsample_ratios=[4, 2, 2, 2], kernel_size=[7, 3, 3, 3], 
        ...
        self.out_channels = (3, 64, 128, 256, 512) # output 4 branches
    ...
    def init_weights(self, pretrained):
        ...
        # 5 branches
        # del state_dict['layers.0.RC.PCM.0.weight']
        # del state_dict['layers.0.RC.PRM.convs.0.0.weight']
        # del state_dict['layers.0.RC.PRM.convs.1.0.weight']
        # del state_dict['layers.0.RC.PRM.convs.2.0.weight']
        # del state_dict['layers.0.RC.PRM.convs.3.0.weight']
    ...
    def forward_features(self, x, Wh, Ww):
        ...
        for i in range(len(self.layers)):
            ...
            #x_out = norm_layer(x)
            outs.append(x.view(b, wh, wh, -1).permute(0, 3, 1, 2))
...
def vitaev2_s():
    ...
    backbone = ViTAE_Window_NoShift_basic(
            img_size=512,
