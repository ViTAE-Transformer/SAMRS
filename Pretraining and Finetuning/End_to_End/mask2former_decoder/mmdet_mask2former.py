...
from .mmdet_maskformer import MMDET_MaskFormerHead
...
class Decoupled_MMDET_Mask2FormerDecoder(MMDET_MaskFormerHead):
    ...
    def __init__(self,
            ...
            positional_encoding=Config(dict(  # SinePositionalEncoding
                num_feats=128, normalize=True)),
            transformer_decoder = Config(dict(  # Mask2FormerTransformerDecoder
        ...
        assert pixel_decoder.encoder.layer_cfg.self_attn_cfg.num_levels == num_transformer_feat_level
    ...
    def _forward_mask_head(self, decoder_out: Tensor, mask_feature: Tensor,
    ...
    def forward(self, x: List[Tensor],
        ...
        # change: memory feature per layer
        #cls_pred_list = []
        fea_out_list = []
        mask_pred_list = []
        decoder_out, mask_pred, attn_mask = self._forward_mask_head(
            query_feat, mask_features, multi_scale_memorys[0].shape[-2:])

        # change: memory feature per layer
        # cls_pred_list.append(cls_pred)
        fea_out_list.append(decoder_out)
        ...
        return fea_out_list, mask_pred_list


class Decoupled_MMDET_Mask2FormerHead(MMDET_MaskFormerHead):
    ...
    def __init__(self,
        ...
        loss_mask=Config(dict(
                    type='mmdet.CrossEntropyLoss',
                    use_sigmoid=True,
                    reduction='mean',
                    ignore_index = args.ignore_label,
                    loss_weight=5.0))
        loss_dice=Config(dict(
                    type='mmdet.DiceLoss',
                    use_sigmoid=True,
                    activate=True,
                    reduction='mean',
    ...
    def _get_targets_single(self, cls_score: Tensor, mask_pred: Tensor,
        ...
        #print('ground truth', gt_labels.shape, gt_masks.shape)

        # sample points
        num_queries = cls_score.shape[0]
        num_gts = gt_labels.shape[0]

        #print('nums', num_queries, num_gts)

        point_coords = torch.rand((1, self.num_points, 2),
                                  device=cls_score.device) # [1, 12544, 2]
        
        #print('point_coords',point_coords.shape)

        # shape (num_queries, num_points)
        #print('mask_pred',mask_pred.shape)
        # # [100,1,56,56], [100,12544,2] -> [100,1,12544] -> [100, 12544]
        mask_points_pred = point_sample(
            mask_pred.unsqueeze(1), point_coords.repeat(num_queries, 1,
                                                        1)).squeeze(1)
        # shape (num_gts, num_points)
        # # [1,1,224,224],[1,12544,2] -> [1,1,12544] -> [1,12544] 
        ...
        # print('sampled_gt_instances', gt_labels.shape, gt_points_masks.shape)
        # print('sampled_pred_instance', cls_score.shape, mask_points_pred.shape)
        ...
        # print('pred_instance',pred_instances)
        # print('gt_instance', gt_instances)
    ...
    def _loss_by_feat_single(self, cls_scores: Tensor, mask_preds: Tensor,
        ...
        # ############## newly add ################
        # loss_cls = torch.tensor([loss_cls]).cuda()
