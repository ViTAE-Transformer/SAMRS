...
def show_rbox_mask(point, rbox, hbox, mask, ax, random_color='None', color=None):
    ...
    ax.plot([rbox[0,0],rbox[1,0],rbox[2,0],rbox[3,0],rbox[0,0]],
    [rbox[0,1],rbox[1,1],rbox[2,1],rbox[3,1],rbox[0,1]],lw=2,color=(1,1,1,1))

    x0, y0 = hbox[0], hbox[1]
    w, h = hbox[2] - hbox[0], hbox[3] - hbox[1]
...
# for vis
vis_dir = "/root/dw/samrs/work_dir/hrsc/Vis_Test_Seg_HBox_RBox_GT/"
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        if args.type == 'center':
            ...
            if args.dataset == 'hrsc':
                gt_hboxes, gt_rboxes, gt_colors, gt_points, gt_labels, error = load_hrsc(img_name, ann_dir)
                # gt_colors: b,3
        ...
        for i in range(len(gt_rboxes)):
            ...
            box_pts = gt_rboxes[i].astype(np.int32)
            draws = cv2.fillPoly(canvas, [box_pts], color=(255, 255, 255))
        ...
        rbox_masks = np.stack(rbox_masks,axis=0) #b, h, w
        rbox_mask_prompts= torch.stack(rbox_mask_prompts,dim=0).cuda()

        predictor.set_image(img)
        # don't use box
        #transformed_boxes = predictor.transform.apply_boxes_torch(hbox_coords, img.shape[:2])
        # use point to predict mask
        predict_masks, qualities, lr_logits = predictor.predict_torch(
            point_coords = None,#gt_points[:,None,:],
            point_labels=None,#input_labels[:, None],
            boxes=None,
            mask_input=rbox_mask_prompts[:,None,:,:],
            multimask_output=False) # 只输出一个mask
        
        predict_masks = predict_masks.squeeze(1) # b, h, w
        qualities = qualities.squeeze(-1) # 每个box_mask生成mask的质量

        gt_points = gt_points.squeeze(-1)
        gt_points = gt_points.cpu().numpy()

        predict_masks = predict_masks.cpu().numpy()
        # intersection
        #sam_masks = rbox_masks * predict_masks

        # mask
        sam_masks = predict_masks
        ...
        if args.instance == 'True':
            ...
            all_rboxmasks.append(rbox_masks)
    ...
    if args.instance == 'True':
        ...
        with open(os.path.join(json_dir, 'sam_ins_rbox.json'), "w") as f:
            json.dump(sam_coco_dict, f)

        with open(os.path.join(json_dir, 'gt_ins_rbox.json'), "w") as f:
    ...
    if args.show == 'True':
        ...
        #         show_rbox_mask(all_points[i][j], all_rboxes[i][j], all_masks[i][j], plt.gca(), random_color='box', color=color)
        #     plt.axis('off')
        #     plt.savefig(vis_dir + 'out_rbox_mask_sam_mask_'+all_names[i]+'.png',bbox_inches='tight', pad_inches = 0)
        #     plt.close()
        ...
        for i in range(len(all_labels)):
            ...
            for j in range(len(all_labels[i])):
                color = all_colors[i][j]
                show_rbox_mask(all_points[i][j], all_rboxes[i][j],all_hboxes[i][j], all_gt_masks[i][j], plt.gca(), random_color='box', color=color)
            plt.axis('off')
